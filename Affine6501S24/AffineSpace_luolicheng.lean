...
-- Assuming zipWith_preserves_length is a lemma stating that List.zipWith preserves the list length.
def vsub_Aff : AffPoint K n → AffPoint K n → AffVector K n
| ⟨ l1, hl1 ⟩, ⟨ l2, hl2 ⟩ =>
  ⟨
    List.zipWith (. - .) l1 l2,
    by simp [hl1, hl2]  -- Assuming the lengths are the same, and zipWith preserves length
  ⟩
...
-- Define addition of a point and a vector
def vadd_Aff {K : Type u} [Field K] {n : ℕ} : AffPoint K n → AffVector K n → AffPoint K n
| ⟨p, p1⟩, ⟨v, v1⟩ =>
  ⟨
    List.zipWith (.+.) p v,
    by simp [p1, v1] -- Assume proof of length using available properties
  ⟩

--instance : VAdd (AffVector K n) (AffPoint K n) := { vadd := vadd_Aff}

-- Scalar multiplication of vectors
def smul_Aff : K → AffVector K n → AffVector K n
| k, ⟨v, v1⟩ =>
  ⟨
    List.map (λ x => k * x) v,
    by simp [v1] -- prove that mapping a function over a list preserves its length
  ⟩

instance : SMul K (AffVector K n) := { smul := smul_Aff n}
